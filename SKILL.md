---
name: lfwin-payment
description: LFWIN酷收银聚合支付接口对接操作指南。使用本 skill 实现或修改 LFWin 聚合支付能力，包括 MD5/RSA 签名、表单 POST、条码支付、扫码支付、H5/JSAPI/小程序支付、统一收银台、订单轮询、异步通知、退款、关单、撤销、分账和支付故障排查。
---
...
| ------------ | -------------------------- | --------------------------------------------------------- | ------------------------ |
| 快速对接通用收银台    | `/index/Payment/pre_order` | 文档未要求 service                                             | `payment-flows.md`       |
| 商户扫顾客付款码     | `/payapi/pay/barcode`      | 推荐 `pay.comm.barcode`                                     | `payment-flows.md`       |
| 顾客扫商户二维码     | `/payapi/trans/kxpay`      | `pay.comm.jspay`                                          | `payment-flows.md`       |
| 指定通道二维码      | `/payapi/pay/qrcode`       | `pay.alipay.qrcode`、`pay.wxpay.qrcode`、`pay.unpay.qrcode` | `payment-flows.md`       |
...
| 分账接收方/订单分账   | `/payapi/pay/*sharing*`    | 各通道前缀的分账 service                                          | `profit-sharing.md`      |
